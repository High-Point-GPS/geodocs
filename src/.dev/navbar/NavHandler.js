    ...
    enableDisplayToggle(){
        ...
        displayToggle.addEventListener('click', () => {
            if(this.focus){
                ...
                global.geotab.addin.hpgpsFilemanager.blur();
            } else {
                displayToggle.innerHTML = 'Blur add-in';
                global.geotab.addin.hpgpsFilemanager.focus(global.api, global.state);
            }
    ...
    listenToHash(){
        ...
        setInterval(()=>{
            if(window.location.hash !== this.storedHash){
                this.storedHash = window.location.hash;
                if(this.storedHash !== '#' + 'hpgpsFilemanager'){
                    geotab.addin.hpgpsFilemanager.blur();
                } else {
                    geotab.addin.hpgpsFilemanager.focus(global.api, global.state);
                }
            }
        }, 100);
    }
