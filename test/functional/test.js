...
describe('User visits addin', () => {
   ...
   // Confirm page displaying after initialized and focus is called
    it('should display root div', async () => {
        ...
        await page.waitForSelector('#hpgpsFilemanager', {
            visible: true
        });
    ...
    it('blur button should blur addin', async () => {
        ...
        let hidden = await page.evaluate( () => {
            let toggled = false;
            let addin = document.getElementById('hpgpsFilemanager');
    ...
    it('should refocus after group filter is selected', async () => {
        let originalFocus = await page.evaluate( () => geotab.addin.hpgpsFilemanager.focus);

        await page.click('#group-toggle-button');
        await page.evaluate( () => {
            geotab.addin.hpgpsFilemanager.count = 0;
            geotab.addin.hpgpsFilemanager.focus = () => { geotab.addin.hpgpsFilemanager.count++; };
        });
        await page.click('#group-use-b4-label');

        let invocations = await page.evaluate( () => {
            return geotab.addin.hpgpsFilemanager.count;
        });
        ...
        await page.evaluate( (addinFocus) => {
            geotab.addin.hpgpsFilemanager.focus = addinFocus;
        }, originalFocus);
    });
